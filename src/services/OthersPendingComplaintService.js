...
class OthersPendingComplaintService {
    ...
    getEmployeeCompaintsDetailsByPaging(data) {
        if (null != Cookies.get('empId')) {
            return axios.get(BASE_URL_API+`/complaint/complaint-search?compStatus=Pending&compTypeDeptId=${Cookies.get('deptId')}&statusCd=A&page=${data.currentPage-1}&size=${data.itemsPerPage}&sort=empCompId asc`)
        } else {
    ...
    getDepartmentDetailsByDeptNamePaging(deptName) {
     ...
     getEmployeeCompaintsByComplaintId(data) {
        if (null != Cookies.get('empId')) {
            return axios.get(BASE_URL_API + `/complaint/complaint-search?compStatus=Pending&compId=${data.empCompIdSearch}&compTypeDeptId=${Cookies.get('deptId')}&statusCd=A&page=${data.currentPage-1}&size=${data.itemsPerPage}&sort=empCompId asc`)
        } else {
            alert("You need to login first")
            window.location.replace(LOGIN_UI_BASE_URL);
        }
    ...
    updateComplaintDetails(complaintdata) {
        ...
        advanceSearchComplaintDetails(data) {
            if (null != Cookies.get('empId')) {
                return axios.post(BASE_URL_API+`/complaint/complaint-adv-search?page=${data.currentPage-1}&size=${data.itemsPerPage}`, data.advSearchComplaints)
            } else {
                alert("You need to login first")
                window.location.replace(LOGIN_UI_BASE_URL);
            }
